...
\author{Marcel Mula Tixier \\ Marc Planas Bosch \\ Jaime Prieto Salcedo}
\date{Maig 2026}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{esquema1}
\caption{Arquitectura del HydraNet: un backbone únic (RegNet + BiFPN) alimenta múltiples caps específics per a cada tasca (detecció d'objectes, semàfors, predicció de carrils). Font: Tesla AI Day 2021 \cite{tesla2021}.}
\end{figure}
...
La solució clàssica (\emph{Inverse Perspective Mapping}) assumeix que el terreny és pla, cosa que falla en pendents, ressalts i obstacles. La solució de Tesla, inspirada en arquitectures com DETR \cite{carion2020} i Lift-Splat-Shoot \cite{philion2020}, utilitza un Transformer \cite{vaswani2017}:
...
\caption{Arquitectura del Video Neural Net: la \emph{feature queue} acumula embeddings dels últims frames sobre la representació en BEV; el \emph{Video Module} els processa per donar coherència temporal a les prediccions. Font: Tesla AI Day 2021 \cite{tesla2021}.}
\end{figure}
...
La idea d'una xarxa neuronal entrenada \emph{end-to-end} per a conducció no és nova: Bojarski et al. \cite{bojarski2016} ja la van demostrar amb PilotNet de NVIDIA a escala reduïda, i el camp ha generat una literatura abundant des d'aleshores \cite{chen2024}. La novetat de Tesla rau en haver-la dut a producció massiva en un sistema comercial desplegat a milions de vehicles.
...
Aquesta versió va eliminar una part substancial del codi C++ del planificador heurístic, segons declaracions públiques de l'empresa. Es tracta d'un dels canvis arquitectònics més grans de la història dels sistemes ADAS comercials.
...
\paragraph{Confiança excessiva.} Un risc ben documentat \cite{koopman2023} és el fenomen de \emph{automation complacency}: els conductors es desconnecten progressivament quan el sistema funciona bé, fet que els fa més lents a reaccionar quan el sistema falla. Aquest risc s'agreuja paradoxalment a mesura que el sistema millora.
